...
class Phoenix_CashOnDelivery_Model_CashOnDelivery extends Mage_Payment_Model_Method_Abstract
{
    ...
    */
    ...
    private $_calculated_total = 0;
    private function getCalculatedTotal() {
        if (!$this->_calculated_total) {
            $this->_calculated_total = 0;
            $quote = Mage::getSingleton('checkout/session')->getQuote();
            if (!$quote->getId()) {
                $quote = Mage::getSingleton('adminhtml/session_quote')->getQuote();
            }
            foreach ($quote->getAllItems() as $item) {
                $this->_calculated_total += $item->getRowTotalInclTax();
            }
        }
        return $this->_calculated_total;
    }

    private function getCurrentCost($config) {
        ...
        foreach (explode(';', $config) as $item) {
            ...
            $prices[$each[0]] = $each[1];
        }
        if (!array_key_exists(0, $prices)) {
            return floatval($config);
        }
        ksort($prices);
        $result = floatval($config);
        foreach ($prices as $key => $val) {
            if ($key <= $this->getCalculatedTotal()) {
                $result = floatval($val);
            } else {
                break;
            }
    ...
    {
        return $this->getCurrentCost($this->getConfigData('inlandcosts'));
    }

    public function getForeignCountryCosts()
    {
        return $this->getCurrentCost($this->getConfigData('foreigncountrycosts'));
    }
