...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if (!$order->getIsVirtual()) {
            ...
            if (count($tracks)) {
                ...
                $page->drawText(Mage::helper('sales')->__('Tracking Number'), 385, $yShipments - 7, 'UTF-8');
    ...
    {
        $ttf = Mage::getStoreConfig('sales_pdf/fonts/regular');
        $full_ttf_path = Mage::getBaseDir('lib') . "/" . $ttf;
        if (file_exists($full_ttf_path)) {
            $font = Zend_Pdf_Font::fontWithPath($full_ttf_path);
        } else {
    ...
    {
        $ttf = Mage::getStoreConfig('sales_pdf/fonts/bold');
        $full_ttf_path = Mage::getBaseDir('lib') . "/" . $ttf;
        if (file_exists($full_ttf_path)) {
            $font = Zend_Pdf_Font::fontWithPath($full_ttf_path);
        } else {
    ...
    {
        $ttf = Mage::getStoreConfig('sales_pdf/fonts/italic');
        $full_ttf_path = Mage::getBaseDir('lib') . "/" . $ttf;
        if (file_exists($full_ttf_path)) {
            $font = Zend_Pdf_Font::fontWithPath($full_ttf_path);
        } else {
