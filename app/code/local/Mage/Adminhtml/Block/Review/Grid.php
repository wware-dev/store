...
class Mage_Adminhtml_Block_Review_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    private function _getStatusOptionArray() {
        $statuses_tmp = Mage::getModel('review/review')
            ->getStatusCollection()
            ->load()
            ->toOptionArray();
        $statuses = array();
        foreach ($statuses_tmp as $status_item) {
            $statuses[] = array(
                'value' => $status_item['value'],
                'label' => Mage::helper('review')->__($status_item['label'])
            );
        }
        return $statuses;
    }
    ...
    {
        $statuses = $this->_getStatusOptionArray();
