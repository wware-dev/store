 ...
 * @version    $Id: EmailAddress.php 277 2012-02-22 07:06:01Z hosokawa $
 */
...
class Zend_Validate_EmailAddress extends Zend_Validate_Abstract
{
     ...
     *
     * This code is broken, but in Japanese Environment, it is needed for keitai email addresses
     *
    ...
    {
        if (Mage::getStoreConfig('customer/email_validation/use_strict')) {
            return $this->isValidStrict($value);
        }
        return $this->isValidPermissive($value);
    }

    private function isValidPermissive($value) {
        if (!is_string($value)) {
    ...
    private function isValidStrict($value) {
        if (!is_string($value)) {
            $this->_error(self::INVALID);
            return false;
        }
