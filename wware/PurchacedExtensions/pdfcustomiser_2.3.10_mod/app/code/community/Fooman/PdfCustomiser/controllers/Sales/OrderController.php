...
class Fooman_PdfCustomiser_Sales_OrderController extends Mage_Sales_OrderController
{
    ...
    public function printInvoiceAction()
    {
        ...
        if (is_int($invoiceId)) {
            ...
            if ($this->_canViewOrder($order)) {
                Mage::getModel('sales/order_pdf_invoice')->renderPdf(null, array($orderId));
            } else {
                $this->_redirect('*/*/history');
            }
        }
    ...
    public function printShipmentAction()
    {
        ...
        if (is_int($shipmentId)) {
            ...
            $order = Mage::getModel('sales/order')->load($orderId);
            if ($this->_canViewOrder($order)) {
                Mage::getModel('sales/order_pdf_shipment')->renderPdf(null, array($orderId));
            } else {
                $this->_redirect('*/*/history');
            }
        }
    ...
    public function printCreditmemoAction()
    {
        ...
        if (is_int($creditmemoId)) {
            ...
            $order = Mage::getModel('sales/order')->load($orderId);
            if ($this->_canViewOrder($order)) {
                Mage::getModel('sales/order_pdf_creditmemo')->renderPdf(null, array($orderId));
            } else {
                $this->_redirect('*/*/history');
            }
        }
