...
class Fooman_Testing_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _createQuote($store, $customerId=1)
    {
        ...
        $johnDoe->setEmail('kristof@fooman.co.nz');
    ...
    public function processInvoice(&$order)
    {
        if ($order->canInvoice()) {
            return Mage::getModel('sales/order_invoice_api')
                ->create($order->getIncrementId(), null, null, true);
        }
    }
...
//        Mage::register('isSecureArea', true);
        ...
        for ($i = 0; $i < 3; $i++) {
            Mage::getModel('eav/entity_store')->loadByEntityStore(
                Mage::getModel('eav/entity_type')->loadByCode('order')->getId(),
                $i
            )->delete();
            Mage::getModel('eav/entity_store')->loadByEntityStore(
                Mage::getModel('eav/entity_type')->loadByCode('invoice')->getId(),
                $i
            )->delete();
            Mage::getModel('eav/entity_store')->loadByEntityStore(
                Mage::getModel('eav/entity_type')->loadByCode('shipment')->getId(),
                $i
            )->delete();
            Mage::getModel('eav/entity_store')->loadByEntityStore(
                Mage::getModel('eav/entity_type')->loadByCode('creditmemo')->getId(),
                $i
            )->delete();
        }
    }
    ...
    /**
     * Logs custom message into 'fooman_customordernumbers.log' file if $filename is not set
     *
    ...
    {
        if (empty($filename)) {
            Mage::log($message, Zend_Log::DEBUG, 'fooman_customordernumbers.log', true);
        } else {
